...
The challenges use the [Ethernaut framework](https://github.com/OpenZeppelin/ethernaut).

## Requirement
[Nodejs](https://nodejs.org/en/download/). We recommend using 10.17.0. Consider using [nvm](https://github.com/nvm-sh/nvm) to switch between nodejs version.

## Deployement

To deploy the challenges instance:
```
npm install
npm start
```
The challenges will be accessible at `localhost:3000` (it might take some time to start).

The source code of the exercises are located in `contracts/levels`.
The scripts solving the challenges are located in `tests/levels/`.


# Original Ethernaut Readme
