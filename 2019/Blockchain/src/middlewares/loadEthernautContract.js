...
export default store => next => action => {
  ...
  Ethernaut.at(state.gamedata.ethernautAddress)
    ...
    .catch(() => {
      console.error(`@bad Ethernaut contract not found in the current network. Please make sure (1) that you are using metamask, (2) that it's on the correct testnet (given in the challenge description), (3) that it is unlocked`)
    })
}
